...
function FilteredPosts(props) {
    ...
    const containsTags = (postTags) => {
        ...
        while (i < tagsArray.length) {
            if (postTags.includes(tagsArray[i]))
                return true;
            i++;
        }
        return false;
    ...
    const filteredPosts = tagsArray.length > 0 ? 
        posts.filter(post => post.node.frontmatter.tags.some(containsTags))
        : posts;

    return (
        <PostGrid showDescription={showDescription} posts={filteredPosts} allImages={allImages} />
    );
}
