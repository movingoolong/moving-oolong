    ...
    steps.push(
      <Grid container spacing={3} alignItems="stretch" justify="center" key={i}>
        <Grid
          ...
          sm={12 / NUM_POSTS_SHOWN}
          key={edges[cur].node.id}
        >
          <PostPreview
            postInfo={edges[cur].node}
        ...
        {cur + 1 < edges.length ? (
          <Grid
            ...
            className={classes.item}
            sm={12 / NUM_POSTS_SHOWN}
            key={edges[cur + 1].node.id}
          >
            <PostPreview
              postInfo={edges[cur + 1].node}
              allImages={data.allFile.edges}
              showDescription={false}
            />
          </Grid>
        ...
        {cur + 2 < edges.length ? (
          <Grid
            ...
            className={classes.item}
            sm={12 / NUM_POSTS_SHOWN}
            key={edges[cur + 2].node.id}
          >
            <PostPreview
              postInfo={edges[cur + 2].node}
              allImages={data.allFile.edges}
              showDescription={false}
            />
          </Grid>
        ...
        {cur + 3 < edges.length ? (
          <Grid
            ...
            className={classes.item}
            sm={12 / NUM_POSTS_SHOWN}
            key={edges[cur + 3].node.id}
          >
            <PostPreview
              postInfo={edges[cur + 3].node}
              allImages={data.allFile.edges}
              showDescription={false}
            />
          </Grid>
